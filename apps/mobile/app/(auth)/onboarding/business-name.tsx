...
export default function BusinessNameScreen() {
  ...
  return (
    <SafeAreaView style={s.safe}><View style={s.container}>
      ...
      <Text style={s.question}>What's your shop called?</Text>
      <Text style={s.hint}>This is what your customers will see</Text>
      <TextInput style={s.input} placeholder="e.g. Sunita's Kitchen" placeholderTextColor={colors.warmGray} value={name} onChangeText={setName} autoFocus />
      <Button title="Next" onPress={() => router.push('/(auth)/onboarding/what-you-sell')} disabled={!name.trim()} />
    </View></SafeAreaView>
  );
}
